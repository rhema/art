		...
		for (int j=0;j<b_index.cols;j++)
		{
			...
			else
			{
				for (int m=0;m<splitNum*splitNum;m++)
				{
					...
					bool isIn = false;
					Point pppp = Point(j,i);
					isIn = isInside(p1,p2,p3,p4,pppp);
					if (isIn)
					{
...
void send_visualization_udp(Mat density_visualization)
{
	printf("Terst...\n");
	...
	//		density_visualization.at<Vec3b>(i,j).val[2]=tProb*highDeisityColor.val[2]+(1-tProb)*lowDensityColor.val[2];
	        printf("%d%s",density_visualization.at<Vec3b>(i,j).val);
		}
	}
}
...
//pCapture = cvCaptureFromAVI("2.avi");   //����������Ƶ�ô˾�  
...
{  
	...
	{  
		...
		send_visualization_udp(density_visualization);
